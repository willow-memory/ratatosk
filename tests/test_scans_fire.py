"""G2-meta-scans — the meta-scan: every AST/grep-guard helper this suite
carries has been shown to catch something, by a test that *calls it*.

*A scan that has never fired has not been shown to check anything.* Every
guard in `tests/test_release_wiring.py` is written to that rule — each has
a planted-violation test beside it — and this file is that rule turned into
a test that reads the *other* test files: it finds every module-level
helper in ``tests/`` that is shaped like a violation-scanner and asserts
that some planted-violation test in the *same file* runs that helper —
directly, or through another helper it calls.
...
copied for its shape and re-grounded against this repo's own test files.
The discovery rule and the plant rule below are theirs, verified here
rather than re-derived, because a meta-scan that disagreed between repos
about what counts as "planted" would be exactly the kind of drift the
fleet plan exists to close. Where a comment below cites a homestead audit
finding, that is where the rule came from; where it cites a file of this
repo's, that is what the rule found here on arrival (2026-09-12):

* `tests/test_release_wiring.py::_names_a_non_suppressed_credential` — the
  credential scan, the guard against jeles' three lost releases — had no
  plant. It was the one module-level scan in this suite that had never
  been shown to fire.
* `tests/test_cassette.py::test_recorder_scrubs_before_it_stores` and
  `tests/test_sync.py::test_deposit_writes_bundle` each asked a membership
  question of a file they had just read, inline in the test body, with
  nothing to name and nothing to plant. Both now delegate to a helper
  planted in the same file.
* `tests/test_seal_watcher.py::_is_seal` is a predicate handed to the
  watcher as a callback, not a scan of the tree; the `_is_` name rule
  reports it all the same, and it is planted rather than allow-listed,
  because an allow-list is where a meta-scan goes to stop meaning anything.
...
* it matches text with a pattern (`re.search`/`findall`/`finditer`/`match`/
  ...
  — `tests/test_release_wiring.py::_packaged_constant` is this shape, **or**
* it reads a file's text (`.read_text()`/`.read_bytes()`) *and* asks a
  membership question of it (`x in text`, `x not in parts`) — the grep shape
  with no regex in it, which `tests/test_cassette.py::_stored_cassette_carries`
  and `tests/test_sync.py::_deposit_carries` both are, **or**
* it walks a **module-level list of forbidden words** and asks membership of
  each — `any(c in str(value) for c in NON_SUPPRESSED_CREDENTIALS)`. This
  is the shape homestead's first draft missed entirely, and it is the shape
  of this repo's one unplanted scan: `_names_a_non_suppressed_credential`
  reads no file (its caller hands it the text), compiles no pattern and
  parses no source, so none of the three rules above see it. The rule is
  narrow on purpose — the iteration must be over a module-level
  *collection* constant and the body must ask an `in` question — because
  ...
  to it and stops meaning anything.
...
a promise that it sees everything a scan could be shaped like. A helper that
is *passed* to something as a callback rather than called (`_is_seal` above,
...
#: Word-stems the fleet's scan helpers use, split on "_" so
...
#: *first*) both match without over-firing on unrelated names. This repo's
#: helpers carry none of these stems today (its scans are found by shape);
...
#: prose about the thing under test — homestead's `test_invariants_release.py`
#: has a docstring reading "no tag workflow fires, nothing publishes", about
#: GitHub's event suppression and not about a plant at all, and it was
#: clearing that file's credential scan (which had no plant — homestead
#: audit, 2026-09-11). This repo's `test_release_wiring.py` is the same port
#: of the same kartikeya file, and its credential scan was unplanted the same
#: way. A word that common in a repo's own subject matter cannot also be its
#: evidence of a plant.
...
#: returns the real file's text, and homestead's `tests/test_server.py` had
#: exactly one scan of it (a `_route_post` slice asserted not to name
#: "schedule") that no rule here could see (homestead audit, 2026-09-11).
...
def _filters_by_membership(node: ast.AST) -> bool:
    ...
    hoisted to the caller: homestead's `tests/_scans.py::terms_found` —
    twenty-one call sites in eight modules — is exactly that and matched none
    of the four shapes (homestead audit, 2026-09-11). Narrow the same way: the
    membership question must be asked *of the loop variable itself*, so
    iterating a table and asserting something about a result is still not a
    scan.
    """
    for sub in ast.walk(node):
...
def _is_fixture(node: ast.FunctionDef) -> bool:
    ...
    exempted the whole decorated test from both halves of this file
    (homestead audit, 2026-09-11). No test here wears the marker today; the
    day one does, it must not be exempt.
    """
...
#: The shared scan module, if a suite has one. Not a `test_*.py` file, so it
#: holds no plant tests of its own and the same-file rule above cannot reach
#: it — and homestead's first pass left theirs swept by nothing at all, while
...
#: module is swept too, against plants anywhere in `tests/` (homestead audit,
#: 2026-09-11). This repo has no `tests/_scans.py` today. The sweep is kept
#: live and planted against a fixture tree below, so the day one appears it
#: is already held to the rule rather than discovered to be exempt.
SHARED_SCANS_NAME = "_scans.py"
SHARED_SCANS = TESTS_DIR / SHARED_SCANS_NAME
...
def _unplanted_shared_scan_helpers(tests_dir: Path = TESTS_DIR) -> list[str]:
    ...
    proves a shared helper belongs with the caller that relies on it. Takes
    the directory so the sweep can be planted against a fixture tree."""
...
def test_the_meta_scan_finds_a_grep_shaped_scan(tmp_path):
    ...
    `in`. The repo has this shape already (`test_cassette.py`'s
    `_stored_cassette_carries`, `test_sync.py`'s `_deposit_carries`), and a
    discovery rule that only knew about `ast` would clear a file whose only
    guard is this shape."""
    source = _write(
        tmp_path,
...
def test_the_plant_check_does_not_fire_on_a_real_guarded_file():
    ...
    `tests/test_release_wiring.py` defines the credential word-list scan,
    the pr-title guard's regex scan over `PACKAGED`, and the config-comment
    filter, and plants each, so the meta-scan must clear it — or it would be
...
# ── the two shapes the first draft could not see (homestead audit) ──────────
...
def test_the_meta_scan_finds_a_regex_shaped_scan(tmp_path):
    ...
    (`banned_spellings`) carrying none of the repo's stems.
    `tests/test_release_wiring.py::_packaged_constant` is this shape."""
    source = _write(
        tmp_path,
...
def test_the_meta_scan_finds_a_forbidden_word_list_scan(tmp_path):
    """Planted: the fourth shape, and the one this repo's only never-fired
    scan is. `tests/test_release_wiring.py::_names_a_non_suppressed_credential`
    walks `NON_SUPPRESSED_CREDENTIALS` asking membership of text its caller
    hands it — it reads no file, compiles no pattern, parses no source, and
    its name carries no stem, so the first three rules all clear it. It is
    the whole guard against GITHUB_TOKEN's suppressed events."""
    source = _write(
        tmp_path,
...
def test_the_word_list_rule_does_not_fire_on_ordinary_constant_use(tmp_path):
    ...
    and a rule loose enough to call it one reports ordinary table-driven
    tests and gets itself allowlisted into silence."""
    source = _write(
        tmp_path,
...
def test_the_meta_scan_finds_a_scan_handed_its_word_list_by_the_caller(tmp_path):
    """Planted: the fifth shape, the one homestead's shared module is.
    `terms_found(haystack, terms)` walks the *caller's* list asking
    ...
    call sites in eight modules leaned on it (homestead audit, 2026-09-11).
    Here, `test_release_wiring.py::_missing_comments` is this shape with the
    list module-level."""
    source = _write(
        tmp_path,
...
def test_the_shared_scan_module_sweep_fires_on_a_planted_tests_tree(tmp_path):
    """Planted: a `tests/` tree with a shared `_scans.py` defining two scan
    helpers, and one test module whose plant calls only one of them. The
    sweep must name the other — cross-module, since the shared module has
    no plant tests of its own — and must clear the tree once a plant
    reaches both. This repo has no shared module yet; this is what the
    sweep does the day it gets one."""
    tests_dir = tmp_path / "tests"
    tests_dir.mkdir()
    (tests_dir / SHARED_SCANS_NAME).write_text(
        "import re\n"
        "_BANNED = re.compile(r'banned')\n"
        "\n"
        "def leaks(path, secret):\n"
        "    return secret in path.read_text(encoding='utf-8')\n"
        "\n"
        "def banned_words(text):\n"
        "    return _BANNED.findall(text)\n",
        encoding="utf-8",
    )
    (tests_dir / "test_one.py").write_text(
        "from tests._scans import leaks\n"
        "\n"
        "def test_the_leak_check_catches_a_planted_leak(tmp_path):\n"
        "    p = tmp_path / 'x'\n"
        "    p.write_text('s3cret', encoding='utf-8')\n"
        "    assert leaks(p, 's3cret')\n",
        encoding="utf-8",
    )
    assert _unplanted_shared_scan_helpers(tests_dir) == ["banned_words"], (
        "the shared helper no plant anywhere reaches must be reported"
    )
    (tests_dir / "test_two.py").write_text(
        "from tests import _scans\n"
        "\n"
        "def test_the_word_scan_fires_on_a_planted_word():\n"
        "    assert _scans.banned_words('a banned word')\n",
        encoding="utf-8",
    )
...
def test_the_shared_scan_module_is_swept_like_every_other_file():
    """Against the real tree: there is no `tests/_scans.py` here today, so
    the sweep has nothing to report — recorded as a fact rather than assumed,
    so a shared module that appears is swept from its first commit."""
    assert not SHARED_SCANS.exists(), (
        "tests/_scans.py now exists: this test's premise has changed, and "
        "the sweep below is now live against real helpers"
    )
...
def test_a_docstring_saying_fires_about_something_else_does_not_clear_a_scan(tmp_path):
    ...
    suppression, not about a plant — was clearing their
    `test_invariants_release.py`'s credential scan, which had no plant at
    all and needed one. Only "plant" counts in a docstring; "fires" and
...
#
# Inline scans the meta-scan cannot see. Everything above reads
# *module-level helpers*. A guard written inline, in a test's own body, with
# nothing to name and nothing to plant, is invisible to it —
# `tests/test_cassette.py`'s "the secret is not in the saved cassette" check
# and `tests/test_sync.py`'s "the deposit carries the turn" check were
# exactly this shape before this bite factored them out (homestead's were
# `test_view.py`'s tkinter check and `test_nestor_seam.py`'s lazy-import
# check). This half closes that gap: it walks every `test_*` function's own
# body for the same four shapes the module-helper half already knows, holds
# it to the same "reads a file, not a string it built" requirement, and
# clears a test that delegates to a scan helper already defined (and already
# required to be planted) somewhere in `tests/` — recognized by name,
# whichever module actually owns it, since `from tests.test_x import
# _helper` and a bare call to a same-file helper are the same delegation by
# a different route.
#
...
# does not. Loosening that would also catch `"version" not in
# pyproject["project"]` — `test_release_wiring.py` asserts exactly that —
# and similar ordinary assertions that have nothing to do with scanning a
# tree, and a rule that cries wolf on those gets an allowlist bolted onto it
# and stops meaning anything, exactly the fate the module docstring above
# warns the word-list shape away from.
...
#: (homestead's `test_server.py` cut it exactly so), and a membership
#: question of it is still a question of the tree. The list is deliberately
#: all `str`/`bytes` methods — `json.loads(...)` is a bare call to a *name*,
...
def _is_inline_scan(
    ...
    **Delegation clears the whole test, deliberately.** A test that calls a
    ...
    question of the text it read, because in homestead's suite that second
    question is the fixture precondition of the first, every time it occurs
    (`test_i44_no_drafting.py` asserts `"Purpose.EXPORT" in source` before
    planting the edit), and a stricter rule would report it. No test here
    leans on it yet; the boundary is kept fleet-identical and pinned below.
    The cost is real and named: an unplanted inline `in` hidden behind a
    delegated call is not seen. The alternative pushes those preconditions
    out of sight, which is worse.
    """
    if not (
...
def _test_functions(tree: ast.Module):
    ...
    a `class Test*`. This suite writes neither today; homestead's
    `test_intake.py` writes eight classes of them, and a rule that read only
    `tree.body` could never see a scan in one."""
    for node in tree.body:
...
def test_the_inline_scan_check_knows_every_spelling_of_reading_a_real_file(tmp_path):
    ...
    by a spelling it had not been shown (homestead audit, 2026-09-11)."""
    spellings = {
...
def test_the_inline_scan_check_follows_a_slice_of_the_text_but_not_a_parse_of_it(
    ...
    wolf on (`test_release_wiring.py` asks one of pyproject)."""
    sliced = _write(
        tmp_path,
...
def test_a_usefixtures_marker_does_not_exempt_a_test_from_either_half(tmp_path):
    ...
    anywhere in the decorator cleared every test wearing it — four of
    homestead's modules set it, and any inline scan under them was
    invisible. The marker must not exempt; a real `@pytest.fixture` beside
    it still must."""
    marked = _write(
        tmp_path,
...
def test_the_inline_scan_check_clears_a_test_that_delegates_to_a_known_helper(tmp_path):
    ...
    source = _write(
        ...
        f"TARGET = Path({str(target)!r})\n"
        "\n"
        "from tests.test_hooks import _toplevel_and_nested_imports\n"
        "\n"
...
def test_the_inline_scan_check_reads_class_methods_and_async_tests_too(tmp_path):
    ...
    suite writes neither yet; homestead's writes eight test classes, and a
    scan in a method was never reportable there. Either would be the same
    blind spot here the day one is written."""
    in_a_class = _write(
        tmp_path,
...
def test_delegating_clears_a_test_that_also_asks_its_own_question(tmp_path):
    ...
    including an `in` of its own on the same text. Three of homestead's
    real tests depend on it — the `in` beside the delegated call is the
    precondition that keeps the plant honest ("the fixture really does carry
    the thing I am about to change"), and reporting those would push
    preconditions out of sight for no gain."""
    source = _write(
        tmp_path,
        "test_planted_delegate_and_check.py",
        "from pathlib import Path\n"
        "from tests.test_cassette import _stored_cassette_carries\n"
        "\n"
        "def test_the_planted_secret_is_reported():\n"
        "    source = Path('x').read_text(encoding='utf-8')\n"
        "    assert 'scrubbed_keys' in source\n"
        "    assert _stored_cassette_carries(Path('x'), 'secret')\n",
    )
    assert _inline_scan_tests(source, frozenset({"_stored_cassette_carries"})) == []
    assert _inline_scan_tests(source, frozenset()) == [
        "test_the_planted_secret_is_reported"
...
def test_a_local_function_sharing_a_helpers_name_does_not_clear_a_scan(tmp_path):
    ...
    shadow = _write(
        ...
        "from pathlib import Path\n"
        "\n"
        "def _deposit_carries(text):\n"
        "    return text\n"
        "\n"
        "def test_no_banned_word():\n"
        "    text = Path('x').read_text(encoding='utf-8')\n"
        "    assert _deposit_carries(text)\n"
        "    assert 'banned' not in text\n",
    )
    assert _inline_scan_tests(shadow, frozenset({"_deposit_carries"})) == [
        "test_no_banned_word"
    ], "a local `_deposit_carries` is not `test_sync.py`'s `_deposit_carries`"

    imported = _write(
        tmp_path,
        "test_planted_imported_helper.py",
        "from pathlib import Path\n"
        "from tests.test_sync import _deposit_carries\n"
        "\n"
        "def test_no_banned_word():\n"
        "    text = Path('x').read_text(encoding='utf-8')\n"
        "    assert _deposit_carries(text)\n"
    ...
    assert _inline_scan_tests(imported, frozenset({"_deposit_carries"})) == [], (
...
def test_the_inline_scan_check_does_not_fire_on_the_real_tree():
    """The whole thing against the two real files of this repo's that used
    to carry an inline scan: `test_cassette.py` (the scrub check) and
    `test_sync.py` (the deposit check) now each delegate to a helper planted
    in the same file — so the check must clear both, or it would be crying
    wolf on the very fix it exists to require."""
    global_helpers = _global_scan_helper_names()
    for name in ("test_cassette.py", "test_sync.py"):
